...
"""
অ্যানিমেথিক বট v6.0 - প্যানেল থেকে সব এডিট করা যায়
✅ আপনার সব কথা মতো
"""
...
async def button_handler(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "dash":
        ...
        text = (
            f"⚙️ **Settings**\n\n"
            f"👋 **Welcome Message:**\n"
            f"{'🟢 চালু' if settings['welcome']['enabled'] else '🔴 বন্ধ'}\n"
            f"[🔴 বন্ধ করো] [🟢 চালু করো]\n\n"
            f"🔗 **Link Filter:**\n"
            f"{'🟢 চালু' if settings['filter']['enabled'] else '🔴 বন্ধ'}\n"
            f"[🔴 বন্ধ করো] [🟢 চালু করো]\n\n"
            f"⚠️ **Max Warnings:** {settings['filter']['max_warnings']}\n"
            f"[➖ কমান] {settings['filter']['max_warnings']} [➕ বাড়ান]\n\n"
            f"🔇 **Mute Duration:** {settings['filter']['mute_duration']} min\n"
            f"[➖ কমান] {settings['filter']['mute_duration']} [➕ বাড়ান]\n\n"
            f"🌐 **Allowed Domains:**\n" + "\n".join([f"• {d}" for d in settings['filter']['domains']]) + "\n\n"
            f"[➕ ডোমেইন যোগ করুন] [❌ ডোমেইন সরান]"
        )
        keyboard = [
            [InlineKeyboardButton("👋 Welcome ON", callback_data="welcome_on"),
             InlineKeyboardButton("👋 Welcome OFF", callback_data="welcome_off")],
            [InlineKeyboardButton("🔗 Filter ON", callback_data="filter_on"),
             InlineKeyboardButton("🔗 Filter OFF", callback_data="filter_off")],
            [InlineKeyboardButton("⚠️ Warn -", callback_data="warn_down"),
             InlineKeyboardButton("⚠️ Warn +", callback_data="warn_up")],
            [InlineKeyboardButton("🔇 Mute -", callback_data="mute_down"),
             InlineKeyboardButton("🔇 Mute +", callback_data="mute_up")],
            [InlineKeyboardButton("➕ Add Domain", callback_data="add_domain"),
             InlineKeyboardButton("❌ Remove Domain", callback_data="remove_domain")],
        ...
        keyboard = [
            [InlineKeyboardButton("🔴 OFF", callback_data="search_off"),
             InlineKeyboardButton("🟢 ON", callback_data="search_on")],
            [InlineKeyboardButton("◀️ পিছনে", callback_data="back")]
        ]
        await q.edit_message_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
    
    elif data == "welcome_on":
        settings["welcome"]["enabled"] = True
        save_json('settings.json', settings)
        await q.edit_message_text("✅ ওয়েলকাম মেসেজ চালু করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
    
    elif data == "welcome_off":
        settings["welcome"]["enabled"] = False
        save_json('settings.json', settings)
        await q.edit_message_text("✅ ওয়েলকাম মেসেজ বন্ধ করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
    
    elif data == "filter_on":
        settings["filter"]["enabled"] = True
        save_json('settings.json', settings)
        await q.edit_message_text("✅ লিংক ফিল্টার চালু করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
    
    elif data == "filter_off":
        settings["filter"]["enabled"] = False
        save_json('settings.json', settings)
        await q.edit_message_text("✅ লিংক ফিল্টার বন্ধ করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
    
    elif data == "warn_up":
        settings["filter"]["max_warnings"] += 1
        save_json('settings.json', settings)
        await q.edit_message_text(f"✅ ম্যাক্স ওয়ার্নিং {settings['filter']['max_warnings']} করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
    
    elif data == "warn_down":
        if settings["filter"]["max_warnings"] > 1:
            settings["filter"]["max_warnings"] -= 1
            save_json('settings.json', settings)
            await q.edit_message_text(f"✅ ম্যাক্স ওয়ার্নিং {settings['filter']['max_warnings']} করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
        else:
            await q.edit_message_text("❌ ১ এর কম হতে পারে না", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
    
    elif data == "mute_up":
        settings["filter"]["mute_duration"] += 5
        save_json('settings.json', settings)
        await q.edit_message_text(f"✅ মিউট ডিউরেশন {settings['filter']['mute_duration']} মিনিট করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
    
    elif data == "mute_down":
        if settings["filter"]["mute_duration"] > 5:
            settings["filter"]["mute_duration"] -= 5
            save_json('settings.json', settings)
            await q.edit_message_text(f"✅ মিউট ডিউরেশন {settings['filter']['mute_duration']} মিনিট করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
        else:
            await q.edit_message_text("❌ ৫ এর কম হতে পারে না", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]]))
        ...
        await q.edit_message_text(
            ...
            parse_mode='Markdown',
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Settings", callback_data="set")]])
        )
        ...
        save_json('settings.json', settings)
        await q.edit_message_text("✅ /search কমান্ড চালু করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Search Settings", callback_data="search")]]))
    
    elif data == "search_off":
        settings["search"]["command"] = False
        save_json('settings.json', settings)
        await q.edit_message_text("✅ /search কমান্ড বন্ধ করা হয়েছে", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Search Settings", callback_data="search")]]))
...
# ========== ডোমেইন ম্যানেজমেন্ট ==========
async def add_domain(u: Update, c: ContextTypes.DEFAULT_TYPE):
    if not is_admin(u.effective_user.id):
        return
    if not c.args:
        await u.message.reply_text("Usage: /adddomain example.com")
        return
    domain = c.args[0].lower().replace('https://', '').replace('http://', '').replace('www.', '')
    if domain not in settings["filter"]["domains"]:
        settings["filter"]["domains"].append(domain)
        save_json('settings.json', settings)
        await u.message.reply_text(f"✅ {domain} যোগ করা হয়েছে")
    else:
        await u.message.reply_text(f"⚠️ {domain} ইতিমধ্যে আছে")

async def remove_domain(u: Update, c: ContextTypes.DEFAULT_TYPE):
    if not is_admin(u.effective_user.id):
        return
    if not c.args:
        await u.message.reply_text("Usage: /removedomain example.com")
        return
    domain = c.args[0].lower().replace('https://', '').replace('http://', '').replace('www.', '')
    if domain in settings["filter"]["domains"]:
        settings["filter"]["domains"].remove(domain)
        save_json('settings.json', settings)
        await u.message.reply_text(f"✅ {domain} সরানো হয়েছে")
    else:
        await u.message.reply_text(f"❌ {domain} পাওয়া যায়নি")
